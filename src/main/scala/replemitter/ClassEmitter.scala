...
private[replemitter] final class ClassEmitter(sjsGen: SJSGen) {
  ...
  def buildClass(tree: ClassDef, useESClass: Boolean, ctor: js.Tree,
    ...
    if (!tree.kind.isJSClass) {
      ...
      val entireClassDefWithGlobals =
        if (useESClass) {
          val parentVarWithGlobals = for (parentIdent <- tree.superClass) yield {
            ...
            else WithGlobals(js.Apply(globalVar("a", parentIdent.name), Nil))
          }
      ...
      val classDefStatsWithGlobals = for {
        ...
        entireClassDef :: createStaticFields
      }
  ...
  def extractInlineableInit(tree: ClassDef): (Option[Versioned[MethodDef]], List[MethodDef]) = {
    (None, tree.memberDefs.collect({
      case m: MethodDef => m}))
  }
  ...
  def genConstructor(tree: ClassDef, useESClass: Boolean,
  ...
  private def genES5Constructor(tree: ClassDef,
      initToInline: Option[MethodDef])(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[js.Tree] = {
    import TreeDSL._
    implicit val pos = tree.pos
  ...
  private def genES6Constructor(tree: ClassDef,
      initToInline: Option[MethodDef])(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[js.Tree] = {
    implicit val pos = tree.pos
  ...
  private def genJSSuperCtor(tree: ClassDef)(
   ...
   */
  private def genJSConstructorFun(tree: ClassDef,
    ...
    val superCtorCallAndFieldDefs = if (forESClass) {
      val fieldDefs = genFieldDefsOfScalaClass(tree.className, tree.memberDefs.collect({
        case f: FieldDef => f
      }))
  ...
  private def genConstructorFunForJSClass(tree: ClassDef)(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[js.Function] = {
    implicit val pos = tree.pos
  ...
  def genCreateStaticFieldsOfScalaClass(tree: ClassDef)(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[List[js.Tree]] = {
    val defs = for {
      field @ FieldDef(flags, FieldIdent(name), origName, ftpe) <- tree.memberDefs.collect {
        case f: AnyFieldDef => f
      }
  ...
  def genCreatePrivateJSFieldDefsOfJSClass(tree: ClassDef)(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[List[js.Tree]] = {
    val defs = for {
      field @ FieldDef(flags, FieldIdent(name), origName, _) <- tree.memberDefs.collect({
        case f: FieldDef => f
      })
      if !flags.namespace.isStatic
    } yield {
  ...
  private def genCreateStaticFieldsOfJSClass(tree: ClassDef)(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[List[js.Tree]] = {
    val className = tree.className
    val statsWithGlobals = for {
      field <- tree.memberDefs.collect {
        case f: FieldDef => f
        case jf: JSFieldDef => jf
      }
  ...
  def genStaticInitialization(tree: ClassDef)(
      implicit globalKnowledge: GlobalKnowledge): List[js.Tree] = {
    implicit val pos = tree.pos
    val hasStaticInit = tree.memberDefs.collect {
      case m: MethodDef => m
    }.exists { m =>
  ...
  private def genClassInitialization(tree: ClassDef)(
      implicit globalKnowledge: GlobalKnowledge): List[js.Tree] = {
    implicit val pos = tree.pos
  ...
  private def hasClassInitializer(tree: ClassDef): Boolean = {
    tree.memberDefs.collect({
      case m: MethodDef => m
    })
      .exists { m =>
  ...
  private def genJSMethod(tree: ClassDef, useESClass: Boolean,
      method: JSMethodDef)(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[js.Tree] = {
    implicit val pos = method.pos
  ...
  private def genJSProperty(tree: ClassDef, useESClass: Boolean,
  ...
  private def genJSPropertyES5(tree: ClassDef, property: JSPropertyDef)(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[js.Tree] = {
  ...
  private def exportTargetES5(tree: ClassDef, namespace: MemberNamespace)(
      implicit globalKnowledge: GlobalKnowledge,
  ...
  private def genSingleInstanceTests(tree: ClassDef)(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[List[js.Tree]] = {
    import TreeDSL._
  ...
  private def genArrayInstanceTests(tree: ClassDef)(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[List[js.Tree]] = {
    import TreeDSL._
  ...
  def genTypeData(tree: ClassDef)(
      implicit globalKnowledge: GlobalKnowledge, knowledgeGuardian: KnowledgeGuardian): WithGlobals[js.Tree] = {
    import TreeDSL._
    ...
    val ancestorsRecord = js.ObjectConstr(
        knowledgeGuardian.calculateAncestors(tree).map(ancestor => (js.Ident(genName(ancestor)), js.IntLiteral(1))))
    ...
    isInstanceFunWithGlobals.flatMap { isInstanceFun =>
      val allParams = List(
          ...
          js.StringLiteral(RuntimeClassNameMapperImpl.map(
              semantics.runtimeClassNameMapper, tree.className.nameString)),
          ancestorsRecord,
          isJSTypeParam,
          parentData,
          isInstanceFun
  ...
  def genSetTypeData(tree: ClassDef)(
    ...
    globalVar("b", tree.name.name).prototype DOT "$classData" :=
      globalVar("d", tree.name.name)
  }

  def genModuleAccessor(tree: ClassDef)(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[js.Tree] = {
    import TreeDSL._
      ...
      val assignModule = {
        moduleInstanceVar := {
          if (tree.kind == ClassKind.JSModuleClass) {
            ...
            js.Apply(globalVar("ct", (className, NoArgConstructorName)), List(js.New(js.Apply(globalVar("a", className), Nil), Nil)))
          }
        }
      }
  ...
  def genExportedMembers(tree: ClassDef, useESClass: Boolean)(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[js.Tree] = {
    val jsMethodPropDefs = tree.memberDefs.collect {
      case m: JSMethodPropDef => m
    }
    val exportsWithGlobals = jsMethodPropDefs map { _ match {
        case m: JSMethodDef =>
  ...
  def genTopLevelExports(topLevelExports: List[TopLevelExportDef])(
      implicit globalKnowledge: GlobalKnowledge): WithGlobals[List[js.Tree]] = {
...
private[replemitter] object ClassEmitter {
  ...
  def shouldExtendJSError(linkedClass: ClassDef): Boolean = {
